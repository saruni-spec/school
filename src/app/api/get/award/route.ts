...
export async function GET(request: Request) {
  try {
    ...
    const records = await prisma.award.findMany({
      where: {
        school_id: Number(school_id),
        OR: [
          { users: { school_id: Number(school_id) } },
          { department: { school_id: Number(school_id) } },
          { class_progression: { stream: { school_id: Number(school_id) } } },
        ],
      },
      select: {
        id: true,
